...
BOOST_AUTO_TEST_CASE(eq) {
    BOOST_STATIC_ASSERT(!Eq<Lit<Fib<1>>, Lit<True>>::value);
    BOOST_STATIC_ASSERT(Eq<Lit<Fib<1>>, Lit<Fib<1>>>::value);
    BOOST_STATIC_ASSERT(!Eq<Lit<Fib<0>>, Lit<False>>::value);
    BOOST_STATIC_ASSERT(!Eq<Lit<Fib<5>>, Lit<Fib<1>>>::value);
}
